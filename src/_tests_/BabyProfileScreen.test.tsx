...
import * as Access from '../services/BabyProfileAccess'

// Mocks
jest.mock('expo-speech', () => ({ speak: jest.fn(), stop: jest.fn() }))
jest.mock('../services/BabyProfileAccess', () => ({
...
function BabyProfileScreenWrapper(
...
describe('BabyProfileScreen', () => {
  it('submits and saves the baby profile', async () => {
    const { getByPlaceholderText, getByText } = render(
      <NavigationContainer>
        <Stack.Navigator>
          <Stack.Screen name="Baby" component={BabyProfileScreenWrapper} />
        </Stack.Navigator>
      </NavigationContainer>
    )

    fireEvent.changeText(getByPlaceholderText('Baby’s name'), 'Luna')
    fireEvent.changeText(getByPlaceholderText('Date of Birth (YYYY-MM-DD)'), '2023-01-01')

    const button = getByText('Next')
    fireEvent.press(button)

    expect(Access.createProfile).toHaveBeenCalled()
  })
})
